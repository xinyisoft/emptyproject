...
PHP_JS.prototype = {
    ...
    var libbcmath = {
      ...
      _one_mult: function (num, n_ptr, size, digit, result, r_ptr) {
        ...
        if (digit === 0) {
          ...
          if (digit == 1) {
            libbcmath.memcpy(result, r_ptr, num, n_ptr, size);
          } else {
            nptr = n_ptr + size - 1;
            rptr = r_ptr + size - 1;
            carry = 0;
  ...
  aggregate: function (obj, class_name) {
    var p = '',
        record = {},
        pos = -1;
  ...
  aggregate_methods: function (obj, class_name) {
    var p = '',
        record = {},
        pos = -1;
    var getFuncName = function (fn) {
    ...
    this.php_js.aggregateClasses.push(getFuncName(class_name));
    for (p in class_name) {
      if (!(p in obj) && typeof class_name[p] === 'function' && p[0] !== '_') {
        obj[p] = class_name[p];
        record[p] = class_name[p];
      }
    }
    for (p in class_name.prototype) {
      if (!(p in obj) && typeof class_name.prototype[p] === 'function' && p[0] !== '_') {
        obj[p] = class_name.prototype[p];
        record[p] = class_name.prototype[p];
      }
    }
  ...
  aggregate_methods_by_list: function (obj, class_name, properties_list, exclude) {
    var p = '',
        i = 0,
        record = {},
        pos = -1;
    ...
    if (exclude) {
      for (p in class_name) {
        if (!(p in obj) && typeof class_name[p] === 'function' && p[0] !== '_' && properties_list.indexOf(p) === -1) {
          obj[p] = class_name[p];
          record[p] = class_name[p];
        }
      }
      for (p in class_name.prototype) {
        if (!(p in obj) && typeof class_name.prototype[p] === 'function' && p[0] !== '_' && properties_list.indexOf(p) === -1) {
          obj[p] = class_name.prototype[p];
          record[p] = class_name.prototype[p];
        }
      }
      ...
      for (i = 0; i < properties_list.length; i++) {
        p = properties_list[i];
        if (!(p in obj) && p in class_name && p[0] !== '_' && typeof class_name.prototype[p] === 'function') {
          obj[p] = class_name[p];
          record[p] = class_name[p];
        } else if (!(p in obj) && p in class_name.prototype && p[0] !== '_' && typeof class_name.prototype[p] === 'function') {
          obj[p] = class_name.prototype[p];
          record[p] = class_name.prototype[p];
        }
      }
  ...
  aggregate_methods_by_regexp: function (obj, class_name, regexp, exclude) {
    var p = '',
        test = false,
        record = {},
        pos = -1,
    ...
    for (p in class_name) {
      ...
      if (!(p in obj) && typeof class_name[p] === 'function' && p[0] !== '_' && test) {
        obj[p] = class_name[p];
        record[p] = class_name[p];
      }
    }
    for (p in class_name.prototype) {
      test = exclude ? p.search(regexp) === -1 : p.search(regexp) !== -1;
      if (!(p in obj) && typeof class_name.prototype[p] === 'function' && p[0] !== '_' && test) {
        obj[p] = class_name.prototype[p];
        record[p] = class_name.prototype[p];
      }
    }
  ...
  aggregate_properties: function (obj, class_name) {
    var p = '',
        record = {},
        pos = -1,
        indexOf = function (value) {
  ...
  aggregate_properties_by_list: function (obj, class_name, properties_list, exclude) {
    var p = '',
        i = 0,
        record = {},
        pos = -1,
  ...
  aggregate_properties_by_regexp: function (obj, class_name, regexp, exclude) {
    var p = '',
        test = false,
        record = {},
        pos = -1,
  ...
  aggregation_info: function (obj) {
    return this.aggregate_info(obj);
  },
  array: function () {
    var mainArgs = arguments,
        p = this.php_js = this.php_js || {},
      ...
      if (!p.PHPJS_Array) {
        ...
        e.list = function () {
          ...
          for (var i = 0, argl = arguments.length; i < argl; i++) {
            var key = _.keys[i];
            if (key && key.length === parseInt(key).toString().length && parseInt(key) < argl) {
  ...
  array_multisort: function (arr) {
    var argl = arguments.length,
        sal = 0,
        flags = {'SORT_REGULAR': 16, 'SORT_NUMERIC': 17, 'SORT_STRING': 18, 'SORT_ASC': 32, 'SORT_DESC': 40},
        sortArrs = [
          []
        ],
        sortFlag = [0],
  ...
  classkit_import: function (file) {
    eval(this.file_get_contents(file));
  },
  classkit_method_add: function (classname, methodname, args, code, flags) {
    var func, argmnts = [];
    switch (flags) {
  ...
  classkit_method_copy: function (dClass, dMethod, sClass, sMethod) {
    ...
    dClass.prototype[dMethod] = sClass.prototype[sMethod];
    return true;
  },
  classkit_method_redefine: function (classname, methodname, args, code, flags) {
    var argmnts = [],
        func;
    switch (flags) {
      case 'CLASSKIT_ACC_PROTECTED':
  ...
  classkit_method_remove: function (classname, methodname) {
    if (typeof classname === 'string') {
      classname = this.window[classname];
    }
  ...
  classkit_method_rename: function (classname, methodname, newname) {
    if (typeof classname === 'string') {
      classname = this.window[classname];
    }
  ...
  define: function (name, value) {
    ...
    try {
      ...
      defn = (this.execScript ? function (name, value) {
  ...
  error_get_last: function () {
    return this.php_js && this.php_js.last_error ? this.php_js.last_error : null;
  },
  error_reporting: function (level) {
  ...
  fclose: function (handle) {
    ...
    try {
      ...
      delete this.php_js.resourceData[handle.id];
    } catch (e) {
      return false;
    }
    return true;
  ...
  fgetcsv: function (handle, length, delimiter, enclosure, escape) {
    var start = 0,
        fullline = '';
    ...
    if (fullline === '') {
      fullline = this.php_js.resourceData[handle.id].slice(start);
    }
    length = (length === undefined || fullline.length < length) ? fullline.length : Math.floor(length / 2) || 1;
    this.php_js.resourceDataPointer[handle.id] += length;
  ...
  fgetss: function (handle, length, allowable_tags) {
    var start = 0,
        fullline = '';
    if (!this.php_js || !this.php_js.resourceData || !this.php_js.resourceDataPointer || length !== undefined && !length) {
      return false;
  ...
  fopen: function (filename, mode, use_include_path, context) {
    var resource = {},
        i = 0,
        that = this;
    var getFuncName = function (fn) {
    ...
    function PHPJS_Resource(type, id, opener) {
      this.type = type;
      this.id = id;
      this.opener = opener;
    }
  ...
  forward_static_call: function (cb, parameters) {
    var func;
    if (typeof cb == 'string') {
  ...
  forward_static_call_array: function (cb, parameters) {
    var func;
    if (typeof cb == 'string') {
      if (typeof this[cb] == 'function') {
        func = this[cb];
  ...
  fpassthru: function (handle) {
    ...
    this.echo(chrs);
    this.php_js.resourceDataPointer[handle.id] = this.php_js.resourceData[handle.id].length;
    return chrs;
  },
  fread: function (handle, length) {
    if (!this.php_js || !this.php_js.resourceData || !this.php_js.resourceDataPointer) {
      return false;
    }
    length = length < 8192 ? (Math.floor(length / 2) || 1) : 4096;
    var start = this.php_js.resourceDataPointer[handle.id];
    if (start === undefined) {
      return false;
    }
  ...
  get_defined_constants: function (categorize) {
    var ext = '',
        cnst = '',
        constObj = {},
        flatConstObj = {},
  ...
  get_extension_funcs: function (module_name) {
    this.php_js = this.php_js || {};
  ...
  get_resource_type: function (handle) {
    var getFuncName = function (fn) {
      var name = (/\W*function\s+([\w\$]+)\s*\(/).exec(fn);
      if (!name) {
        return '(Anonymous)';
  ...
  gmdate: function (format, timestamp) {
    var dt = typeof timestamp === 'undefined' ? new Date() : typeof timestamp === 'object' ? new Date(timestamp) : new Date(timestamp * 1000);
  ...
  ini_alter: function (varname, newvalue) {
  ...
  ini_get_all: function (extension, details) {
    var key = '',
        ini = {},
        noDetails = {},
        extPattern;
  ...
  mail: function (to, subject, message, additional_headers, additional_parameters) {
  ...
  metaphone: function (word, phones) {
    ...
    function removeDuplicates(word) {
      ...
      for (var i = 1; i < wordlength; i++) {
        char2 = word.charAt(i);
        if (char2 !== char1 || char2 === 'C' || char2 === 'G') {
          rebuilt += char2;
        }
        char1 = char2;
      }
  ...
  ob_get_contents: function () {
    ...
    var phpjs = this.php_js,
        ...
        obs = phpjs.obs;
    if (!obs || !obs.length) {
      return (ini && ini.output_buffering && (typeof ini.output_buffering.local_value !== 'string' || ini.output_buffering.local_value.toLowerCase() !== 'off')) ? '' : false;
    }
  ...
  ob_get_length: function () {
    ...
    var phpjs = this.php_js,
        ...
        obs = phpjs.obs;
    if (!obs || !obs.length) {
      return (ini && ini['output_buffering'] && (typeof ini['output_buffering'].local_value !== 'string' || ini['output_buffering'].local_value.toLowerCase() !== 'off')) ? 0 : false;
    }
  ...
  ob_get_status: function (full_status) {
    var i = 0,
        retObj = {},
        ob = {},
        retArr = [],
  ...
  ob_start: function (output_callback, chunk_size, erase) {
    var bufferObj = {},
        internalType = false,
        extra = false;
    erase = !(erase === false);
    ...
    this.php_js = this.php_js || {};
    this.php_js.obs = this.php_js.obs || [];
    var phpjs = this.php_js,
        ini = phpjs.ini,
        obs = phpjs.obs;
  ...
  runkit_class_adopt: function (classname, parentname) {
    if (typeof this.window[classname] !== 'function' || typeof this.window[parentname] !== 'function') {
      return false;
    }
    this.window[classname].prototype = new this.window[parentname]();
    this.window[classname].constructor = this.window[classname];
    return true;
  },
  runkit_class_emancipate: function (classname) {
  ...
  runkit_function_add: function (funcname, arglist, code) {
    if (this.window[funcname] !== undefined) {
      return false;
    }
    try {
  ...
  runkit_function_copy: function (funcname, targetname) {
    if (typeof this.window[funcname] !== 'function' || this.window[targetname] !== undefined) {
      return false;
    }
    this.window[targetname] = this.window[funcname];
    return true;
  },
  runkit_function_redefine: function (funcname, arglist, code) {
    if (this.window[funcname] === undefined) {
      return false;
    }
    try {
  ...
  runkit_function_remove: function (funcname) {
    if (this.window[funcname] === undefined) {
      return false;
    }
    try {
  ...
  runkit_function_rename: function (funcname, newname) {
  ...
  runkit_import: function (file, flags) {
    if (flags) {
      throw 'Flags not supported for runkit_import';
    }
    eval(this.file_get_contents(file));
  },
  runkit_method_add: function (classname, methodname, args, code, flags) {
    var func, argmnts = [];
    switch (flags) {
  ...
  runkit_method_copy: function (dClass, dMethod, sClass, sMethod) {
    sMethod = sMethod || dMethod;
    if (typeof dClass === 'string') {
      dClass = this.window[dClass];
    }
  ...
  runkit_method_redefine: function (classname, methodname, args, code, flags) {
    var argmnts = [],
        func;
    var getFuncName = function (fn) {
      var name = (/\W*function\s+([\w\$]+)\s*\(/).exec(fn);
    ...
    if (getFuncName(classname) !== 'PHP_JS' || (this.php_js && this.php_js.ini && this.php_js.ini['runkit.internal_override'] && (this.php_js.ini['runkit.internal_override'].local_value === true || this.php_js.ini['runkit.internal_override'].local_value === 1 || this.php_js.ini['runkit.internal_override'].local_value === '1' || this.php_js.ini['runkit.internal_override'].local_value === 'true'))) {
      func = Function.apply(null, argmnts.concat(code));
      classname.prototype[methodname] = func;
      return true;
    }
  ...
  runkit_method_remove: function (classname, methodname) {
    if (typeof classname === 'string') {
      classname = this.window[classname];
    }
    if (getFuncName(classname) !== 'PHP_JS' || (this.php_js && this.php_js.ini && this.php_js.ini['runkit.internal_override'] && (this.php_js.ini['runkit.internal_override'].local_value === true || this.php_js.ini['runkit.internal_override'].local_value === 1 || this.php_js.ini['runkit.internal_override'].local_value === '1' || this.php_js.ini['runkit.internal_override'].local_value === 'true'))) {
  ...
  runkit_method_rename: function (classname, methodname, newname) {
    var getFuncName = function (fn) {
      var name = (/\W*function\s+([\w\$]+)\s*\(/).exec(fn);
      if (!name) {
        return '(Anonymous)';
  ...
  set_include_path: function (new_include_path) {
    this.php_js = this.php_js || {};
    this.php_js.ini = this.php_js.ini || {};
  ...
  setlocale: function (category, locale) {
    ...
    if (!phpjs.locales) {
      ...
      phpjs.locales.en = {
        ...
        'LC_MESSAGES': {YESEXPR: '^[yY].*', NOEXPR: '^[nN].*', YESSTR: '', NOSTR: ''},
  ...
  stream_context_create: function (options, params) {
    var resource = {};
    options = options || {};
    this.php_js = this.php_js || {};
    this.php_js.resourceIdCounter = this.php_js.resourceIdCounter || 0;
  ...
  stream_context_get_default: function (options) {
    this.php_js = this.php_js || {};
  ...
  stream_context_get_options: function (stream_or_context) {
    return stream_or_context.stream_options;
  },
  stream_context_get_params: function (stream_or_context) {
    return stream_or_context.stream_params;
  },
  stream_context_set_default: function (options) {
    this.php_js = this.php_js || {};
    if (!this.php_js.default_streams_context) {
      this.php_js.default_streams_context = this.stream_context_create(options);
    }
  ...
  stream_context_set_option: function (stream_or_context, optionsOrWrapper, option, value) {
    if (option) {
      if (!stream_or_context.stream_options[optionsOrWrapper]) {
  ...
  stream_context_set_params: function (stream_or_context, params) {
    var param = '';
    for (param in params) {
      if (param === 'options') {
        stream_or_context.stream_options = params[param];
      } else {
  ...
  stream_get_contents: function (handle, maxLength, offset) {
    if (!this.php_js || !this.php_js.resourceData || !this.php_js.resourceDataPointer || !handle || !handle.id) {
      return false;
    }
    offset = offset || 0;
  ...
  stream_get_line: function (handle, length, ending) {
    var start = 0,
        fullline = '';
    if (!this.php_js || !this.php_js.resourceData || !this.php_js.resourceDataPointer || length !== undefined && !length) {
      return false;
  ...
  stream_is_local: function (stream_or_url) {
    if (typeof stream_or_url === 'string') {
      return ((/^(https?|ftps?|ssl|tls):/).test(stream_or_url)) ? false : true;
    }
  ...
  timezone_abbreviations_list: function () {
    ...
    for (i = 0, len = php_js_shared.tz_abbrs.length; i < len; i++) {
      ...
      for (j = 0, jlen = curr.length; j < jlen; j++) {
        ...
        dst = !!currSub[0];
        list[indice].push({'dst': dst, 'offset': tzo, 'timezone_id': timezone_id});
        if (dtz === timezone_id) {
  ...
  trigger_error: function (error_msg, error_type) {
    var type = 0,
        i = 0,
        that = this,
        prepend = '',
    ...
    if (type & TYPES.E_USER_ERROR || type & TYPES.E_ERROR || type & TYPES.E_CORE_ERROR || type & TYPES.E_COMPILE_ERROR || type & TYPES.E_RECOVERABLE_ERROR || type & TYPES.E_PARSE) {
      ...
      var e = new Error(error_msg);
      e.type = type;
      this.php_js.last_error = {message: e.message, file: e.fileName, line: e.lineNumber, type: e.type};
      throw e;
    }
    if (display_errors(type)) {
      switch (type) {
  ...
  xdiff_string_diff: function (old_data, new_data, context_lines, minimal) {
    var i = 0,
        ...
        new_hunk_size, MAX_CONTEXT_LINES = Number.POSITIVE_INFINITY,
        MIN_CONTEXT_LINES = 0,
    ...
    while (i < ori_len || j < new_len) {
      ...
      if (k >= lcs_len || trailing_context.length >= 2 * context_lines) {
  ...
  xdiff_string_patch: function (originalStr, patch, flags, error) {
    var getNativeFlags = function (regex) {
        ...
        cbSplit = function (str, s) {
          ...
          var x = s._xregexp;
          s = new RegExp(s.source, getNativeFlags(s) + 'g');
          if (x) {
            s._xregexp = {source: x.source, captureNames: x.captureNames ? x.captureNames.slice(0) : null};
          }
        ...
        errors = '',
        optTemp = 0,
        OPTS = {'XDIFF_PATCH_NORMAL': 1, 'XDIFF_PATCH_REVERSE': 2, 'XDIFF_PATCH_IGNORESPACE': 4};
